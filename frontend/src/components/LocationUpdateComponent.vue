<template>
  ...
  <form class="g-3" @submit.prevent="handleUpload" v-if="item">
    <div class="mb-3">
    ...
    <div class="mb-3">
      <label for="addressStart" class="form-label">
        Địa chỉ : <em>{{item.address.ward.name}}, {{item.address.district.name}}, {{item.address.province.name}}</em>
      </label>
      <div id="addressStart">
        <select id="provincesStart" name="provinceSender" class="form-select mb-3" v-model="item.address.province.name" @change="updateCityId">
          <option value="" disabled selected>Chọn tỉnh thành</option>
          <option v-for="(city, index) in cities" :value="city.name" :key="index" :data-id="city.id">{{ city.name }}</option>
        </select>
        <select id="districtsStart" name="districtSender" class="form-select mb-3" v-model="item.address.district.name" @change="updateDistrictId">
        ...
        <select id="wardsStart" name="wardSender" class="form-select mb-3" v-model="item.address.ward.name" @change="updateWardId">
...
export default {
  ...
  data(){
    return{
      ...
      updateItem:{
        province: '', district: '',ward: '' , provinceCode: '', districtCode: '', wardCode: '', name:'', phone:'',
      }
    }
  },
  methods:{
    ...
    async handleUpload(){
      this.updateItem = {
        ...
        ward: this.item.address.ward.name, wardCode: this.item.address.ward.code,
        district: this.item.address.district.name, districtCode: this.item.address.district.code,
        province: this.item.address.province.name, provinceCode: this.item.address.province.code,
        type: this.type
      }
      const response = await MerchandiseService.updateOneMerchandiseAddress(this.id, this.updateItem);
      if (response.status === 200) {
        this.resultMessage = { type: 'success', message: 'Cập nhật địa chỉ thành công' };
